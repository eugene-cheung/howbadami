...
> **Where this file lives:** `npx getdesign add cursor` wrote the first copy to `..\..\DESIGN.md` relative to the folder you ran it from (for `…/Projects/howbadami`, that resolves to `C:\Users\skyne\DESIGN.md`). This file is the same content, kept **in the repository** so agents and teammates can find it next to the code.
